...
contract ComptrollerG3 is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function _become(Unitroller unitroller, uint platformTokenRate_, address[] memory platformTokenMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == unitroller.admin(), "only unitroller admin can change brains");
        require(unitroller._acceptImplementation() == 0, "change not authorized");

        ComptrollerG3(address(unitroller))._becomeG3(platformTokenRate_, platformTokenMarketsToAdd, otherMarketsToAdd);
    }

    function _becomeG3(uint platformTokenRate_, address[] memory platformTokenMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == comptrollerImplementation, "only brains can become itself");

        for (uint i = 0; i < platformTokenMarketsToAdd.length; i++) {
            _addMarketInternal(address(platformTokenMarketsToAdd[i]));
        }
        ...
        _setPlatformTokenRate(platformTokenRate_);
        _addPlatformTokenMarkets(platformTokenMarketsToAdd);
    }
    ...
    function refreshPlatformTokenSpeeds() public {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            if (markets[address(gToken)].isPlatformTokened) {
                ...
                Exp memory interestPerBlock = mul_(Exp({mantissa: gToken.borrowRatePerBlock()}), gToken.totalBorrows());
