...
let getGTokenAddresses = (cTokenArgs) => {
	let all = [
		'cUSDC',
		'cDAI',
		'cUSDT',
...
let filterInitialized = async (borrowersByGToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling compBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let cTokenAddr of Object.keys(borrowersByGToken)) {
		let speed = await call(Comptroller, 'compSpeeds', [cTokenAddr]);
		if (Number(speed) != 0){
			for (let borrowerChunk of getChunks(borrowersByGToken[cTokenAddr], batchSize)) {
				try {
...
let filterBorrowers = (apiAccounts, cTokenList) => {
	return apiAccounts.reduce((acc, account) => {
		let validBorrowers = account.tokens.filter(
			(accountGToken) =>
				cTokenList.includes(accountGToken.address) &&
				accountGToken.borrow_balance_underlying.value > 0
		);
...
let claimCompBatch = async (borrowersByGToken, opts) => {
	for (let cTokenAddr of Object.keys(borrowersByGToken)) {
		let borrowers = borrowersByGToken[cTokenAddr];
...
(async () => {
	let borrowersByGToken;
	let cTokenMap; // symbol => addrs
	let opts = getConfig(args[0]);
	if (network == 'development') {
		borrowersByGToken = getTestData();
	} else if (isKnownNetwork(network)) {
		let apiAccounts = opts.readFixture
			? await readFixture()
			: await accountRequest(network, opts);
		let cTokenAddresses = Object.values(getGTokenAddresses(opts.cTokens));
		borrowersByGToken = filterBorrowers(apiAccounts, cTokenAddresses);
		if (opts.writeFixture) await writeFixture(apiAccounts);
	} else {
		printUsage();
	}
	let unInit = await filterInitialized(borrowersByGToken);
	print('Uninitialized accounts before: ', unInit);

	await claimCompBatch(unInit, opts);
	unInit = await filterInitialized(borrowersByGToken);
