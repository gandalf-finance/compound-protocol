        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (GToken Eth Address) (... deploy gToken)`
              // could be used to deploy a gToken if it doesn't exist, but
              // since there is no GToken, that check would raise (when we'd
