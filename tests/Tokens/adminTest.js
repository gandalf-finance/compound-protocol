...
describe('admin / _setPendingAdmin / _acceptAdmin', () => {
  ...
  describe('_setPendingAdmin()', () => {
    it('should only be callable by admin', async () => {
      expect(
        await send(gToken, '_setPendingAdmin', [accounts[0]], {from: accounts[0]})
      ).toHaveTokenFailure(
        'UNAUTHORIZED',
  ...
  describe('_acceptAdmin()', () => {
    it('should fail when pending admin is zero', async () => {
      expect(
        await send(gToken, '_acceptAdmin')
      ).toHaveTokenFailure(
        'UNAUTHORIZED',
    ...
    it('should succeed and set admin and clear pending admin', async () => {
      expect(await send(gToken, '_setPendingAdmin', [accounts[0]])).toSucceed();
      expect(await send(gToken, '_acceptAdmin', [], {from: accounts[0]})).toSucceed();
    ...
    it('should emit log on success', async () => {
      expect(await send(gToken, '_setPendingAdmin', [accounts[0]])).toSucceed();
      const result = await send(gToken, '_acceptAdmin', [], {from: accounts[0]});
